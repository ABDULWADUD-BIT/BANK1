...
void loan() {
    if (choice == 7) {
        ...
        printf("Our interest rate is 10%%\n");
        printf("Enter your name: ");
        scanf("%s", name);
        printf("Enter your ID: ");
        scanf("%lld", &id);
        ...
        while (fscanf(ptr, "%s %s %s %lld %lld %lld", s1.my_name, s1.fa_name, s1.ma_name, &s1.id, &s1.pass, &s1.balance) != EOF) {
            if (strcmp(s1.my_name, name) == 0 && s1.id == id) {
                ...
                printf("What amount of money do you need: ");
                scanf("%lld", &x);

                long long int given_amount = 0;
                if (x > 100000) {
                    given_amount = x - (x * 0.02); // 2% processing fee
                    printf("The amount you wanted was: %lld\n", x);
                    printf("Due to bank policy (2%% processing cut):\n");
                    printf("We can give you: %lld\n", given_amount);
                } else {
                    given_amount = x;
                    printf("We can give you full amount: %lld\n", given_amount);
                }

                printf("Do you want the loan? (1 = Yes, 0 = No): ");
                int confirm;
                scanf("%d", &confirm);

                if (confirm == 1) {
                    s1.balance += given_amount;
                    long long int interest = given_amount * 0.10;

                    printf("Loan approved!\n");
                    printf("Updated Balance: %lld\n", s1.balance);
                    printf("You have to repay: %lld (including 10%% interest)\n", given_amount + interest);

                    FILE *npp = fopen("sf.txt", "a");
                    FILE *top = fopen("loan.txt", "a");
                    fprintf(npp, "%s %lld %lld\n", s1.my_name, s1.id, s1.balance);
                    int t;
                    t = given_amount + interest;
                    fprintf(top, "%s %lld %lld\n", s1.my_name, s1.id, t);
                    fclose(npp);
                    fclose(top);
                } else {
            ...
            // Save the current record (whether modified or not)
            fprintf(temp, "%s %s %s %lld %lld %lld\n",
                s1.my_name, s1.fa_name, s1.ma_name, s1.id, s1.pass, s1.balance);
        }
